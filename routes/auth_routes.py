...
def dashboard():
    ...
    if status_filter:
        if status_filter == 'Vacant':
            ...
            employees_to_show = [emp for emp in employee_rows if emp.get('Status') == status_filter]
    ...
    locations = {}
    if employee_rows:
        locations = {emp['Accommodation']: sum(1 for e in employee_rows if e.get('Accommodation') == emp.get('Accommodation')) for emp in employee_rows}
